...
pdf := $(patsubst tests/%.md,docs/%.pdf,$(test))
...
pandocArgFragment := $(pandocArgCommon) --top-level-division=part --filter=$(filter)
### pandoc workflow
...
README := docs/index.html README.md

# Main Targets ########################################################################################################################################################################################

all: $(native) $(pdf) $(README)
readme: $(README)
travis: $(native)
...
docs/%.pdf: tests/%.md $(filter)
	pandoc -F $(filter) -o $@ $<

clean:
	rm -f $(native) $(pdf) $(README)

# Making dependancies #################################################################################################################################################################################

# readme
## docs
docs/index.html: tests/README.md
